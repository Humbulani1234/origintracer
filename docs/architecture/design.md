...
Probes call `emit()` which appends to `EventBuffer` - a thread-safe bounded deque. The drain thread processes the buffer asynchronously. Request handlers never wait for graph processing.
