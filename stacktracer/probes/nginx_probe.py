...
# ── Private BPF C source ──────────────────────────────────────────────────────
# This string is private to this module.
# It is NOT re-exported. Users who have not purchased the nginx probe
# package will not have this file and therefore this string.
#
# Rules observed (required for correct compilation alongside other probes):
#   - Does NOT redeclare kernel_event_t  (defined in BRIDGE_BPF_HEADER)
#   - Does NOT redeclare BPF_PERF_OUTPUT (defined in BRIDGE_BPF_HEADER)
#   - Does NOT redeclare trace_context   (defined in BRIDGE_BPF_HEADER)
#   - All maps prefixed nginx_           (avoids collisions with other probes)
#   - epoll_wait tracepoints handle BOTH nginx and asyncio paths — asyncio
#     probe does NOT attach its own epoll tracepoints (one attachment per tp)
...
# Callers never see _NGINX_BPF directly; they only see that "nginx" is registered.
