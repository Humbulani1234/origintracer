...
def query(sock_path: str, query_str: str) -> dict:
    ...
    try:
        ...
        return {"ok": False, "error": f"Socket gone: {sock_path} — did the worker restart?"}
    except socket.timeout:
        return {"ok": False, "error": "Query timed out (>10 s)"}
    except ConnectionRefusedError:
        return {"ok": False, "error": "Connection refused — worker may have restarted"}
...
def render(result: dict) -> None:
    ...
    if verb == "CAUSAL":
        ...
        for m in matches:
            ...
            wrapped = textwrap.fill(
                m["explanation"], width=72,
                initial_indent="        ", subsequent_indent="        ",
            )
            print(c(wrapped, DIM))
            evidence = m.get("evidence", {})
            if evidence:
                print(c(f"        Evidence: {json.dumps(evidence)}", DIM))
            print()
        return
    ...
    if metric == "graph" or (isinstance(data, dict) and "nodes" in data and "edges" in data):
        ...
        if edges:
            ...
            for e in edges:
                ...
                suffix = (
                    f" [{etype}" + (f" ×{cnt}" if cnt else "") + "]"
                    if etype or cnt else ""
                )
                print(
                    c(f"  {src}", YELLOW) +
                    c(f"  →  ", DIM) +
    ...
    # ── Fallback: raw JSON ────────────────────────────────────────
    print(json.dumps(data if data is not None else result, indent=2, default=str))
...
def _render_table(rows: list) -> None:
    ...
    print()
    header_line = "  " + "  ".join(
        c(k.upper().replace("_", " "), BOLD, DIM).ljust(widths[k] + 11)
        for k in keys
    )
    print(header_line)
...
def cmd_help() -> None:
    ...
    cmds = [
        ...
        ('\\emit <probe> <svc> <name>',  'Inject a synthetic event into the live engine'),
        ('\\snapshot [label]',           'Capture a temporal diff snapshot'),
        ('\\active',                     'In-flight requests currently being traced'),
        ('\\reconnect',                  'Pick a different worker socket'),
        ('\\help',                       'This message'),
        ('\\quit  or  Ctrl+C',           'Exit'),
    ]
