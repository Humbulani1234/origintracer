...
def engine():
    ...
    from stacktracer.core.semantic import SemanticLayer, SemanticAlias
    from stacktracer.sdk.emitter import bind_engine

    sem = SemanticLayer()
    sem.register(SemanticAlias(
        label="export",
...
def make_event(probe: str, service: str = "django", name: str = "test", trace_id: str = "trace-1", **meta):
...
class TestNormalizedEvent:
    ...
    def test_creation(self):
        from stacktracer.core.event_schema import NormalizedEvent
        e = NormalizedEvent(probe="request.entry", service="django", name="/api/export", trace_id="t1")
    ...
    def test_factory_method(self):
        from stacktracer.core.event_schema import NormalizedEvent
        e = NormalizedEvent.now("asyncio.loop.tick", "t1", "asyncio", "step", foo="bar")
    ...
    def test_repr(self):
        from stacktracer.core.event_schema import NormalizedEvent
        e = NormalizedEvent(probe="request.entry", service="django", name="/api", trace_id="abc123xyz")
...
class TestPatternRegistry:
    ...
    def test_register_and_match(self):
        from stacktracer.core.causal import PatternRegistry, CausalRule
        from stacktracer.core.runtime_graph import RuntimeGraph
        from stacktracer.core.temporal import TemporalStore

        def always_match(g, t):
    ...
    def test_loop_starvation_rule(self):
        ...
        g.upsert_node("asyncio::asyncio.loop.tick", "asyncio", "asyncio", duration_ns=50_000_000)
        g.upsert_node("asyncio::asyncio.loop.tick", "asyncio", "asyncio", duration_ns=50_000_000)
...
class TestEngine:
    ...
    def test_process_builds_graph(self, engine, trace_id):
        e = make_event("function.call", "django", "handle_export", trace_id=trace_id)
        engine.process(e)
    ...
    def test_hotspots_sorted_by_call_count(self, engine, trace_id):
        for _ in range(5):
            engine.process(make_event("function.call", "django", "hot_fn", trace_id=trace_id))
        engine.process(make_event("function.call", "django", "cold_fn", trace_id=trace_id))
...
class TestQueryExecutor:
    ...
    def test_show_latency_returns_data(self, engine, trace_id):
        engine.process(make_event("function.call", "django", "view", trace_id=trace_id, duration_ns=5_000_000))
        from stacktracer.query.parser import parse, execute
        result = execute(parse('SHOW latency WHERE service = "django"'), engine)
        assert "data" in result
        assert any(r["service"] == "django" for r in result["data"])

    def test_hotspot_executor(self, engine, trace_id):
        for _ in range(3):
            engine.process(make_event("function.call", "django", "busy_view", trace_id=trace_id))
        from stacktracer.query.parser import parse, execute
    ...
    def test_trace_executor(self, engine, trace_id):
        engine.process(make_event("request.entry", "django", "/api", trace_id=trace_id))
        engine.process(make_event("request.exit", "django", "/api", trace_id=trace_id))
        from stacktracer.query.parser import parse, execute
...
class TestAsyncioProbe:
    ...
    def test_patch_is_applied(self):
        from stacktracer.probes.asyncio_probe import AsyncioProbe, _STEP_ATTR
        from asyncio import tasks
        probe = AsyncioProbe()
        original = getattr(tasks.Task, _STEP_ATTR)
        probe.start()
        patched = getattr(tasks.Task, _STEP_ATTR)
        assert original is not patched
        probe.stop()
        restored = getattr(tasks.Task, _STEP_ATTR)
