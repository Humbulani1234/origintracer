...
class TestNormalizedEventNow:
    ...
    def test_duration_ns_lands_on_field(self):
        e = NormalizedEvent.now(
            "django.view.exit", "t1", "django", "view",
    ...
    def test_pid_lands_on_field(self):
        e = NormalizedEvent.now(
            "gunicorn.worker.fork", "t1", "gunicorn", "worker",
            pid=12345,
        )
        assert e.pid == 12345
    ...
    def test_tid_lands_on_field(self):
        e = NormalizedEvent.now(
            "asyncio.task.start", "t1", "asyncio", "task",
            tid=99,
        )
        assert e.tid == 99
    ...
    def test_unknown_kwargs_go_to_metadata(self):
        e = NormalizedEvent.now(
            "request.entry", "t1", "nginx", "upstream",
            method   = "GET",
    ...
    def test_mixed_known_and_unknown_kwargs(self):
        ...
        e = NormalizedEvent.now(
            "db.query.end", "t1", "postgres", "SELECT",
...
class TestNormalizedEventSerialisation:
    ...
    def test_to_dict_contains_all_top_level_fields(self):
        e = self._make()
        d = e.to_dict()
        for field in ("probe", "service", "name", "trace_id", "span_id",
                      "duration_ns", "pid", "tid", "wall_time", "timestamp",
                      "metadata"):
    ...
    def test_duration_ns_not_duplicated_in_metadata_after_round_trip(self):
        """
...
class TestNormalizedEventRepr:
    ...
    def test_repr_contains_probe(self):
        e = NormalizedEvent(probe="request.entry", service="django",
                            name="/api", trace_id="abc123xyz")
        assert "request.entry" in repr(e)

    def test_repr_contains_partial_trace_id(self):
        e = NormalizedEvent(probe="p", service="s", name="n", trace_id="abc123xyz")
