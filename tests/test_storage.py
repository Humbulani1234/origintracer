...
from stacktracer.storage.repository import InMemoryRepository, BaseRepository
...
def run_event_contract(repo: BaseRepository):
    ...
    e = evt(probe="request.entry", service="django", name="/api/orders/", trace_id="t-001")
    repo.insert_event(e)
...
class TestInMemoryRepository:
    ...
    def test_get_latest_snapshot_returns_none_before_insert(self):
...
class TestEventRepositoryPostgres:
    ...
    def test_latest_snapshot_survives_multiple_inserts(self, pg_repo):
        ...
        assert row["data"] in (b"v1", b"v2")  # either could be latest depending on timing