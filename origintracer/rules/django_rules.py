...
LATENCY_THRESHOLD_NS = 200_000_000  # 200ms


def _post_deployment_regression(
    graph: RuntimeGraph,
    temporal: TemporalStore,
    tracker=None,
) -> Tuple[bool, Dict]:
    ...
    for diff in temporal._diffs:
        added_edges |= diff.added_edge_keys

    if not added_edges:
        return False, {}

    # find nodes involved in new edges that have high latency
    regressed = []
    for node in graph.all_nodes():
        avg_ns = node.metadata.get("avg_duration_ns", 0)
        if avg_ns < LATENCY_THRESHOLD_NS:
            continue
        # check if this node appears in any new edge
        involved = any(node.id in edge for edge in added_edges)
        if involved:
            regressed.append(
                {
                    "node": node.id,
    ...
    return True, {"regressed_nodes": regressed}


POST_DEPLOYMENT_REGRESSION = CausalRule(
    name="post_deployment_regression",
    description=(
        "New call edges appeared after deployment and the nodes involved "
        "show average latency above 200ms. The new dependency introduced "
        "in this deployment is the probable root cause."
    ),
    predicate=_post_deployment_regression,
    confidence=0.82,
    tags=["deployment", "latency", "regression"],
)

DUPLICATE_RATIO = 1.5  # write fires > 1.5x per view call
WRITE_VERBS = ("INSERT", "UPDATE", "DELETE")


def _is_write_query(node) -> bool:
    node_id_upper = node.id.upper()
    return any(v in node_id_upper for v in WRITE_VERBS)


def _duplicate_transaction(
    graph: RuntimeGraph,
    temporal: TemporalStore,
    tracker=None,
) -> Tuple[bool, Dict]:
    ...
    for node in graph.all_nodes():
        if not _is_write_query(node):
            continue
        if node.call_count < 2:
            continue

        for edge in graph.callers(node.id):
            caller = graph._nodes.get(edge.source)
            if caller is None or caller.call_count <= 0:
                continue
                ...
                duplicates.append(
                    {
                        ...
                        "ratio": round(ratio, 2),
                        "hint": (
                            "Check for double save() calls, missing "
                            "transaction guards, or retry logic without "
                            "idempotency keys."
                        ),
                    }
                )
    ...
    duplicates.sort(key=lambda d: d["ratio"], reverse=True)
    return True, {"duplicate_writes": duplicates[:10]}


DUPLICATE_TRANSACTION = CausalRule(
    name="duplicate_transaction",
    description=(
        "A write query (INSERT/UPDATE/DELETE) fires more than 1.5x "
        "per view invocation. Likely a double save(), missing transaction "
        "guard, or retry without idempotency. Check the caller."
    ),
    predicate=_duplicate_transaction,
    confidence=0.83,
    tags=["db", "correctness", "django", "duplicate"],
)
