"""
Observes gunicorn using  configuration callbacks.

Approach:
    gunicorn has a server hooks system - callback functions you
    define in gunicorn.conf.py. These are the stable extension
    points.
...
def ot_on_starting(server: Any) -> None:
    ...
    # worker. ot_post_fork then drains it into the worker's real engine.
...
def ot_worker_exit(server: Any, worker: Any) -> None:
    """
...
def ot_worker_int(worker: Any) -> None:
    """
...
def ot_worker_abort(worker: Any) -> None:
    """
...
def ot_pre_exec(server: Any) -> None:
    """
...
# -------- install_hooks: programmatic injection into gunicorn server ------


def install_hooks() -> None:
    """
    Injects OriginTracer callbacks into the gunicorn server hook system.

    Call from gunicorn.conf.py:
        from stacktracer.probes.gunicorn_probe import install_hooks
        install_hooks()

    This chains OriginTracer callbacks with any hooks the user has already
    ...
    _chain(config_module, "on_starting", ot_on_starting)
    _chain(config_module, "post_fork", ot_post_fork)
    _chain(config_module, "worker_exit", ot_worker_exit)
    _chain(config_module, "worker_int", ot_worker_int)
    _chain(config_module, "worker_abort", ot_worker_abort)
    _chain(config_module, "pre_exec", ot_pre_exec)

    logger.info("gunicorn OriginTracer hooks installed")
...
# ------------ GunicornProbe: for use with OriginTracer probe registry ------
...
class GunicornProbe(BaseProbe):
    ...
    Pattern 1 - setup in gunicorn.conf.py (recommended):
        from origintracer.probes.gunicorn_probe import install_hooks
        install_hooks()

    Pattern 2 - explicit hook assignment in gunicorn.conf.py:
        from origintracer.probes.gunicorn_probe import (
            ot_on_starting, ot_post_fork, ot_worker_exit,
            ot_worker_int, ot_worker_abort, ot_pre_exec,
        )
        on_starting = ot_on_starting
        post_fork = ot_post_fork
        worker_exit = ot_worker_exit
        worker_int = ot_worker_int
        worker_abort = ot_worker_abort
        pre_exec = ot_pre_exec
    ...
    def stop(self) -> None:
        # Hooks are config-time declarations.
